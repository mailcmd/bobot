...
defapi :smi do
  ...
  defcall :find_user, message do
    url =
      "#{@url}def.get_dev_info.php?action=findUser&decfile=&devkey=&fname=undefined&ifindex=&json=1&code=#{message}"
      |> URI.encode()
      |> URI.encode(&(&1 != 35))

    result =
      Tesla.client([{Tesla.Middleware.BasicAuth, %{username: @username, password: @password}}])
      |> Tesla.get(url)
  ...
  defp process_ftth(user) do
    ...
    [{table1, "#{name} (#{address})", [], []}, {table2, "Señal", [], [{1, [align: :right]}]}]
    |> tables_render()
    |> render_monospaced()
  end
    ...
    tables2 =
      if user[:docsIfCmtsCmStatusValue] =~ "registrationComplete" do
        case docsis_signal(signal_url) do
          :error -> "Lo siento, no pude obtener los valores de señal 😢"
          tables -> tables |> tables_render() |> render_monospaced()
        end
      else
        ""
      end
  ...
  defp docsis_signal(url) do
    url = "#{@url}#{url}&json=1" |> URI.encode() |> URI.encode(&(&1 != 35))
      ...
      down_table =
        {signals[:down]
         |> Enum.map(fn {_, sig} ->
           [
             sig[:ifName],
             "#{sig[:docsIfSigQSignalNoise]} dBm",
             "#{sig[:docsIfDownChannelPower]} dBmV"
           ]
         end), "Calidad de Señal Downstream", ["Down", "SNR", "Power"],
      ...
      up_table =
        {signals[:up]
         |> Enum.map(fn {_, sig} ->
           [
             sig[:ifName],
             "#{sig[:docsIf3CmtsCmUsStatusSignalNoise]} dBm",
             "#{sig[:docsIf3CmStatusUsTxPower]} dBmV"
           ]
         end), "Calidad de Señal Upstream", ["Down", "SNR", "Power"],
         [{1, [align: :right]}, {2, [align: :right]}]}
